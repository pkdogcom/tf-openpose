...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Tensorflow Openpose Video Estimation')
    ...
    with tf.Session() as sess:
        ...
        while True:
            logging.debug('video read+')
            grabbed, img = cap.read()

            if not grabbed:
                break

            logging.debug('video preprocess+')
            if args.zoom < 1.0:
